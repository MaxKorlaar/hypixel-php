<?php
    ...
    class Leaderboards extends HypixelObject
    {
        function get_data($url) {
            ...
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $timeout);
            curl_setopt($ch, CURLOPT_TIMEOUT, $timeout);
            curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.91 Safari/537.36');
            ...
            if (!curl_errno($ch)) {
                ...
                $connect_error = [curl_errno($ch), curl_error($ch)];
                curl_close($ch);
                return false;
            }
        }
        public function getLeaderboard($game) {
            $tnt = "";
            if ($game =="wizard") {
                $game= "tnt"; $tnt = "/wizard";
            }
            ...
            if ($game =="bowspleef") {
                $game= "tnt"; $tnt = "/bowspleef";
            }
            $fetch_url = "http://hypixel.net/games/" . $game . "/Leaderboard" .$tnt;
            $result    = get_data($fetch_url);
            if ($result !== false) {
                $dom = new DomDocument();
                @$dom->loadHTML($result);
                ...
                for ($i = $items->length; --$i >= 0;) {
                    ...
                    if($value != '') {
                        ...
                        foreach($ranklist as $rank) {
                            $value = str_replace($rank, "\n", $value);
                        }
                        $l = explode("\n", $value);
                        if (isset($l[7])) {
                            if($l[7] != '') {
                                array_push($leaderboards, $l[7]);
                            }
                        }
                    }
                }
                $leaderboards = array_reverse($leaderboards);
                return $leaderboards;
            }
            return false;
        }
